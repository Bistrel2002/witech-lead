...
export default function AdminPanel({ apiHost }) {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--wt-rail-bg)] px-4 font-sans text-[var(--wt-rail-fg)]">
        <div className="max-w-md w-full space-y-6 bg-[var(--wt-rail-bg)] border border-[var(--wt-rail-line)] p-8 rounded-2xl shadow-2xl">
          <div className="text-center">
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-[var(--wt-danger)]/10 text-[var(--wt-danger)] mb-4 border border-[var(--wt-danger)]/20">
          ...
          <form onSubmit={handleVerifyPassword} className="space-y-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-3 py-2.5 bg-[var(--wt-rail-bg)] border border-[var(--wt-rail-line)] rounded-lg text-[var(--wt-rail-fg)] text-sm focus:outline-none focus:ring-2 focus:ring-[var(--wt-danger)]/20 focus:border-[var(--wt-danger)]/80 transition-all placeholder:text-fg"
            ...
            <button
              type="submit"
              disabled={verifying}
              className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-lg bg-[var(--wt-danger)] hover:bg-[var(--wt-danger)] text-white font-semibold text-sm shadow-lg shadow-red-900/20 transition-all cursor-pointer disabled:opacity-50"
            >
  ...
  return (
    <div className="min-h-screen bg-surface-2 text-fg font-sans p-6 lg:p-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-line pb-6 mb-8">
        <div>
          <div className="flex items-center gap-2 text-fg-muted text-sm font-semibold uppercase tracking-wider">
            <Shield className="w-4 h-4 text-[var(--wt-danger)]" />
      ...
      {opMessage && (
        <div className={`p-4 rounded-xl border mb-8 flex items-start gap-3 text-sm ${opMessage.type === 'success' ? 'bg-[var(--wt-success-soft)] border-line text-[var(--wt-success-fg)]' : 'bg-[var(--wt-danger-soft)] border-line text-[var(--wt-danger-fg)]'}`}>
          {opMessage.type === 'success' ? <CheckCircle className="w-5 h-5 shrink-0 text-[var(--wt-success)]" /> : <AlertTriangle className="w-5 h-5 shrink-0 text-[var(--wt-danger)]" />}
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-surface border border-line rounded-2xl p-6 shadow-sm">
            <div className="flex items-center justify-between border-b border-line pb-4 mb-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-display font-extrabold text-lg text-fg">Database Security Pipeline</h3>
                  <p className="text-xs text-fg-muted">Backups symétriques chiffrés via Fernet / SHA-256</p>
                </div>
              </div>
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-accent hover:bg-accent disabled:bg-line text-white text-sm font-semibold rounded-lg shadow-md hover:shadow-lg active:translate-y-0.5 cursor-pointer transition-all"
              >
                {backupRunning ? (
                  <>
            ...
            <div className="space-y-4">
              <h4 className="text-sm font-bold text-fg">Archives de Sauvegardes Disponibles</h4>
              
              {loadingBackups ? (
                <div className="py-8 text-center text-fg-subtle text-sm">Chargement des archives de sauvegarde...</div>
              ) : backups.length === 0 ? (
                <div className="py-8 text-center border border-dashed border-line rounded-xl text-fg-subtle text-sm">
                ...
                <div className="overflow-hidden border border-line rounded-xl">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-surface-2 border-b border-line text-fg-muted text-xs font-bold uppercase tracking-wider">
                    ...
                    <tbody className="divide-y divide-line text-sm text-fg-muted">
                      {backups.map((bk) => (
                        <tr key={bk.backup_name} className="hover:bg-surface-2/50">
                          ...
                          <td className="p-3 text-xs"><span className="px-2 py-0.5 rounded-full bg-surface-2 font-semibold">{bk.db_type}</span></td>
                          <td className="p-3 text-xs font-bold">{bk.total_rows}</td>
                          <td className="p-3 text-right pr-4">
                            <button
                              onClick={() => handleRestoreBackup(bk.backup_name)}
                              disabled={!!restoringBackup}
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-line hover:bg-[var(--wt-danger-soft)] text-[var(--wt-danger)] text-xs font-bold rounded-md cursor-pointer transition-colors"
                            >
        ...
        <div className="bg-surface border border-line rounded-2xl p-6 shadow-sm">
          <div className="flex items-center gap-3 border-b border-line pb-4 mb-5">
            ...
            <div>
              <h3 className="font-display font-extrabold text-lg text-fg">Gestion des Comptes</h3>
              <p className="text-xs text-fg-muted">Rôles utilisateurs et droits du CRM</p>
            </div>
          </div>

          <div className="space-y-4">
            {loadingUsers ? (
              <div className="py-8 text-center text-fg-subtle text-sm">Chargement des comptes...</div>
            ) : users.length === 0 ? (
              <div className="py-8 text-center text-fg-subtle text-sm">Aucun utilisateur enregistré.</div>
            ) : (
              <div className="space-y-3">
                {users.map((u) => (
                  <div key={u.id} className="flex items-center justify-between p-3.5 border border-line rounded-xl hover:border-line transition-all">
                    <div>
                      <p className="text-sm font-semibold text-fg">{u.name}</p>
                      <p className="text-xs text-fg-subtle">{u.email}</p>
                      <div className="flex items-center gap-1.5 mt-1.5">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${u.role === 'admin' ? 'bg-[var(--wt-danger-soft)] text-[var(--wt-danger)] border border-line' : 'bg-surface-2 text-fg-muted'}`}>
                          {u.role}
                        </span>
                        {u.google_id && <span className="text-[10px] bg-accent-soft text-accent px-1.5 py-0.5 rounded-full font-semibold">Google</span>}
                        {u.apple_id && <span className="text-[10px] bg-[var(--wt-rail-bg)] text-white px-1.5 py-0.5 rounded-full font-semibold">Apple</span>}
                      </div>
                    </div>
                    ...
                    {u.role !== 'admin' && (
                      <button
                        ...
                        className="p-2 border border-line hover:border-line hover:bg-[var(--wt-danger-soft)] text-fg-subtle hover:text-[var(--wt-danger)] rounded-lg cursor-pointer transition-colors"
                      >
